...
export async function generateRecommendation(exceptionId: number, actorId = 2) {
  ...
  const configured = guidance[String(row.rule_code)] ?? {
    recommendation: 'Compare the source row with authoritative servicing evidence and request correction if the discrepancy cannot be resolved.',
    confidence: 0.78
  };
  ...
  const model = process.env.OPENAI_API_KEY ? (process.env.OPENAI_MODEL ?? 'configured-model') : 'veritas-rules-v1';
  ...
  const [rec] = await sql`
    ...
    VALUES (${Number(row.loan_row_id)},${exceptionId},${explanation},${configured.recommendation},${patch},${configured.confidence},${String(row.severity)},${model},${prompt},${now})
    RETURNING *
  `;
  await audit({
    ...
    metadata: { recommendationId: Number(rec.id), model, confidence: configured.confidence }
  });
  return rec;
}
